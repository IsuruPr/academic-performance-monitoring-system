...
import { generateStudyPlan, chatWithAI } from "./ai.service.js";
...
app.get("/api/optimizer/plan", (req, res) => {
  ...
  // Use user-supplied targetGpa if present and valid, otherwise fall back to mock
  const resolvedTargetGpa = (targetGpa !== undefined && !isNaN(parseFloat(targetGpa)))
    ? parseFloat(targetGpa)
    : target?.targetGpa;

  if (resolvedTargetGpa === undefined || resolvedTargetGpa === null) {
    return res.status(404).json({ message: "No target GPA found" });
  }
